...
class Pipe:
    ...
    async def pipe(self, body, __event_emitter__,
        ...
        try:
            ...
            # Grace timer — if no meaningful event arrives within this window
            # after the last one, assume the stream ended but we missed the
            # lifecycle end, and self-close.
            GRACE_AFTER_LAST_EVENT_S = 8

            while not done:
                try:
                    # Use a shorter timeout after the first event has arrived
                    # (the next one should come quickly if there's more to say)
                    ...
                    pipe_log(f"Event #{event_count}: stream={stream} "
                             f"phase={phase} name={name} state={state}")

                    # --- Completion signals (multiple sources) ---
                    # 1) Lifecycle end/error
                    ...
                    if stream == "assistant":
                        ...
                        if delta:
                            text_yielded = True
                            # Strip inbound metadata blocks that OpenClaw
                            # injects — catch both the prefixed form and
                            # any embedded occurrences
                    ...
                    # --- Grace timer for missing lifecycle events ---
                    # If we got events but the lifecycle end never arrived,
                    # self-close GRACE_AFTER_LAST_EVENT_S after the last
                    # meaningful activity.
                    if (not done and first_event_arrived
                            and (time.time() - last_activity_time
                                 > GRACE_AFTER_LAST_EVENT_S)):
                        pipe_log("  GRACE TIMER — no activity for "
                                 f"{GRACE_AFTER_LAST_EVENT_S}s, self-closing")
                        done = True

                except asyncio.TimeoutError:
                    timeout_desc = "15s" if first_event_arrived else "60s"
                    pipe_log(f"TIMEOUT — no events for {timeout_desc}")
                    # Self-close gracefully instead of hanging forever
                    done = True
