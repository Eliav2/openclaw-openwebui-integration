...
def _assert_pipe_in_sync() -> None:
    """Refuse to deploy a stale built file when the dev source tree is present.

    The deployed artifact is ``openclaw_pipe.py``, generated by ``build.py`` from
    ``src/openclaw_pipe_pkg/``. If those exist, rebuild in-memory and compare; a
    mismatch means someone edited the built file by hand or forgot to rebuild.
    When only the built artifact is present (e.g. remote ``uv run install.py``),
    there is nothing to check and we deploy it as-is.
    """
    ...
    if mod.build() != PIPE_FILE.read_text():
        raise SystemExit(
            "openclaw_pipe.py is out of date vs src/openclaw_pipe_pkg/.\n"
            "Rebuild before deploying:  python3 build.py"
        )
