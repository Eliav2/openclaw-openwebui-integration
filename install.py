...
def http_post(path, data=None, token=None):
    """POST JSON to OWUI API, return parsed response."""
...
def main():
    ...
    step(2, "Removing existing pipe (if any)")

    resp = http_delete(f"/api/v1/functions/id/openclaw_gateway/delete", token)
    if resp.get("detail") and "404" not in str(resp.get("detail")):
        warn(f"Delete response: {resp.get('detail', 'ok')}")
    else:
        info("Existing function removed")
    ...
    if resp.get("id") != "openclaw_gateway":
        print(f"  ✗ Create failed: {resp.get('detail', str(resp))}")
        sys.exit(1)
    info("Pipe function created")

    # ── Toggle active + global ─────────────────────────────────────────
    step(4, "Enabling pipe")

    http_post(f"/api/v1/functions/id/openclaw_gateway/toggle", token=token)
    http_post(f"/api/v1/functions/id/openclaw_gateway/toggle/global", token=token)
    info("Pipe is active + global")

    # ── Generate device identity ───────────────────────────────────────
    step(5, "Generating device identity")

    ident = generate_device_identity()
    device_id = ident["id"]
    ident_json = json.dumps(ident, separators=(",", ":"))

    # Check if the identity was already approved (from a previous run)
    ...
    if os.path.exists(identity_file):
        with open(identity_file) as f:
            saved = json.load(f)
        if saved.get("id") == device_id:
            info(f"Re-using existing identity: {device_id[:20]}...")
        else:
            info(f"New identity: {device_id[:20]}...")
    else:
        info(f"New identity: {device_id[:20]}...")
    ...
    # ── Set valves ─────────────────────────────────────────────────────
    step(6, "Configuring valves")

    resp = http_post(
        f"/api/v1/functions/id/openclaw_gateway/valves/update",
        data={
    ...
    if resp.get("GATEWAY_URL") != GATEWAY_URL:
        print(f"  ✗ Valve update failed: {resp.get('detail', str(resp)[:200])}")
        sys.exit(1)
    info("Valves configured")

    # ── Guide user through device approval ─────────────────────────────
    step(7, "Device pairing")

    # Store the identity in the pipe's media server directory so the pipe
    # can re-use it without needing the valve value. But the valve value
    # is already set above — this is a fallback.
    print()
